...
- A **tier** is a named preset that bundles (model, thinking effort). The only active tier is `default` (9Router CodeBuddy CN DeepSeek V4 Flash at max); normal dispatches should omit `--tier`. Legacy effort flags are rejected because effort is fixed by the registry. The worker discovers provider catalogs dynamically, but the default model remains pinned. See the dated [model decision](../../docs/MODEL_DECISION.md).
